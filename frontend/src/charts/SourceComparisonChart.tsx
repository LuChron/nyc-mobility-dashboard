...
export function SourceComparisonChart({ data }: SourceComparisonChartProps) {
  const option: EChartsOption = {
    ...
    xAxis: { type: 'category', data: data.map((d) => d.metric), ...axisStyle },
    yAxis: { type: 'value', axisLabel: { color: colors.muted, fontSize: 10 }, splitLine: axisStyle.splitLine },
    series: [
      { name: 'Yellow Taxi', type: 'bar', barMaxWidth: 20, data: data.map((d) => d.yellow) },
      { name: 'Green Taxi', type: 'bar', barMaxWidth: 20, data: data.map((d) => d.green) },
      { name: 'HVFHV', type: 'bar', barMaxWidth: 20, data: data.map((d) => d.hvfhv) },
    ],
  };
